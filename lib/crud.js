...
export function crudRouter(db, resource, form, media, all = {}) {
  ...
  if (has('GET')) {
    r.get('/', guardRead, (req, res) => {
      ...
      // Clamped below at 1 — SQLite reads a negative LIMIT as "no limit", so
...
function insert(db, t, data) {
  const keys = Object.keys(data)
  // All-optional resource, nothing sent: "INSERT () VALUES ()" is a SQLite
  // syntax error, but an empty create is still a valid create.
