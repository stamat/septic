...
// json fields (and image fields, which store a metadata blob) are stored as
// TEXT; parse them back to objects on the way out.
function hydrateRow(resource, row) {
  const jf = resource.fields.filter((f) => f.type === 'json' || f.type === 'image').map((f) => f.name)
