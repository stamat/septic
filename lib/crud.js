...
export function crudRouter(db, resource, form, media, all = {}) {
  ...
  if (has('PUT')) {
    r.put('/:id', upload, guardWrite, asyncH(async(req, res) => {
      ...
      try {
        // JSON is partial (only what was sent moves); the edit form posts every
        // field, so it validates as a whole and applies no defaults.
        const row = store.update(id, req.body || {}, { user: req.user, partial: !html })
