...
import { jsonFields } from './schema.js'
...
export function crudRouter(db, resource, form, media) {
  const r = Router()
  ...
  const fileFields = resource.fields.filter((f) => f.type === 'file' || f.type === 'image').map((f) => f.name)
  ...
  const mergeFiles = async(req) => {
    for (const n of fileFields) {
      ...
      if (f) req.body[n] = await saveUpload(f, media)
    }
  }
  ...
  if (has('GET')) {
    ...
    r.get('/:id', guardRead, (req, res) => {
      ...
      res.json(req.query.expand ? expandRow(db, resource, out, req.query.expand) : out)
    })
  }
...
// json fields are stored as TEXT; parse them back to objects on the way out.
function hydrateRow(resource, row) {
  const jf = jsonFields(resource)
  if (!row || !jf.length) return row
  const out = { ...row }
  for (const k of jf) {
    if (typeof out[k] === 'string') { try { out[k] = JSON.parse(out[k]) } catch { /* leave raw */ } }
  }
  return out
}
