...
import { formHtml, listHtml, wantsHtml } from './forms.js'
...
export function crudRouter(db, resource, form, media, all = {}, notify = null) {
  const r = Router()
  ...
  if (has('GET')) {
    r.get('/', guardRead, (req, res) => {
      ...
      const rows = store.list({ user: req.user, limit: lim, offset: off, sort, order, expand, where })
      // The list is the admin's front door, on the same negotiation as the
      // edit form below: a writer's browser GET gets rows it can click into,
      // everyone else — and every API client — keeps the JSON.
      if (form && wantsHtml(req) && allows(req.user, resource.access.write)) {
        return res.send(listHtml(resource, form, { db, rows, limit: lim, offset: off }))
      }
      res.json(rows)
    })
  ...
  if (has('DELETE')) {
    r.delete('/:id', guardWrite, (req, res) => {
      run(res, () => { store.remove(req.params.id, { user: req.user }); notify?.('delete', t, { id: Number(req.params.id) }); res.status(204).end() })
    })
  }
...
// An anonymous browser asking for a page it may not have is a person who has
// not logged in yet — send them to the login page carrying where they were
// going. JSON clients, and browsers logged in but short on role, keep the code.
const deny = (res) => {
  const req = res.req
  if (!req.user && wantsHtml(req)) return res.redirect(303, `/api/_auth/login?next=${encodeURIComponent(req.originalUrl)}`)
