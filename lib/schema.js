...
export function parseResource(name, def) {
  return {
    ...
    uniques: (def.unique || []).map(normalizeUnique)
  }
}

// A unique constraint is a list of columns. The object form adds `coalesce` so a
// nullable column counts a NULL as a sentinel — SQLite treats NULLs as distinct,
// so `UNIQUE(collection, slug)` lets two null-collection pages share a slug;
// `{ columns:["collection","slug"], coalesce:{collection:0} }` makes them collide.
function normalizeUnique(u) {
  if (typeof u === 'string') return { columns: [u], coalesce: {} }
  if (Array.isArray(u)) return { columns: u, coalesce: {} }
  return { columns: u.columns, coalesce: u.coalesce || {} }
}

const sqlLiteral = (v) => typeof v === 'number' ? String(v) : `'${String(v).replace(/'/g, "''")}'`
...
export function createTable(db, resource) {
  ...
  for (const u of resource.uniques) {
    const exprs = u.columns.map((c) => c in u.coalesce ? `COALESCE("${c}", ${sqlLiteral(u.coalesce[c])})` : `"${c}"`)
    db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS "ux_${resource.name}_${u.columns.join('_')}" ON "${resource.name}" (${exprs.join(', ')})`)
  }
...
// The auth `users` table carries these without any config declaring them; a
// config resource extending that table must not hear about them as drift.
const AUTH_OWNED = new Set(['email', 'role', 'password_hash'])
...
// The other direction is drift, and it warns: a column no field declares is
// what a removed or renamed field leaves behind. Never dropped — the data may
// be the only copy, and a rename needs a real migration — but silence here
// would let a renamed field read as "my data vanished" with nothing to go on.
export function ensureColumns(db, resource) {
  ...
  const declared = new Set(resource.fields.map((f) => f.name))
  for (const f of resource.fields) {
    if (!existing.has(f.name)) db.exec(`ALTER TABLE "${resource.name}" ADD COLUMN "${f.name}" ${TYPE_SQL[f.type]}`)
  }
  for (const col of existing) {
    if (col === 'id' || declared.has(col)) continue
    if (resource.name === 'users' && AUTH_OWNED.has(col)) continue
    console.warn(`💩 septic: table "${resource.name}" has a column "${col}" no field declares — left alone; drop or migrate it yourself`)
  }
}
